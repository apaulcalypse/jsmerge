...
from jsmerge.progress import NullProgress
...
def resolve_yang_dir(
    *,
    ...
    if yang_dir is not None:
        ...
        module_count = sum(1 for _ in yang_dir.glob("*.yang"))
        if progress is not None:
            progress.finish(f"Using local YANG directory ({module_count} modules)")
        return yang_dir
